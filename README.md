...
Elixir layer calls the Erlang `itb3` module directly and adds no FFI
...
27+, rebar3, and Elixir 1.17+. macOS: the same via Homebrew; libitb3
builds as `libitb3.dylib`.
...
defp deps do
  [
    {:libitb3_elixir, path: "/path/to/itb/bindings/elixir"}
  ]
end
```
...
:ok = ITB.save_f(sender, "session.blob")       # written by libitb3, mode 0600
...
`ITB.inspect/1` returns the record as a binary-keyed map decoded with
...
such blobs via `ITB.load_f/1`. `itb3` also encrypts / decrypts
